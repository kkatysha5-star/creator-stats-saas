...
router.post('/', async (req, res) => {
  try {
    const { name, username, avatar_color, video_plan_count, video_plan_period, reach_plan } = req.body;
    ...
    const result = await db.execute({ sql: 'INSERT INTO creators (name, username, avatar_color, video_plan_count, video_plan_period, reach_plan) VALUES (?, ?, ?, ?, ?, ?)', args: [name, username || null, color, video_plan_count || 0, video_plan_period || 'month', reach_plan || 0] });
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await db.execute({ sql: 'UPDATE creators SET name = ?, username = ?, avatar_color = ?, video_plan_count = ?, video_plan_period = ?, reach_plan = ? WHERE id = ?', args: [name || e.name, username ?? e.username, avatar_color || e.avatar_color, video_plan_count ?? e.video_plan_count, video_plan_period ?? e.video_plan_period, reach_plan ?? e.reach_plan, req.params.id] });
